...
// ajouter le type propriete

add_action('init', "save_propriete");

function save_propriete(){
	register_post_type('propriete', array(
		'labels'=>array('name'=>'Propriétés ', 
...
require('inc/panneau-custom.php');

add_action("widgets_init", "ajout_widget");

function ajout_widget() {

	register_sidebar(
		array(
			'name'          => "Sidebar pour afficher les maisons",
			'id'            => 'sidebar-2',
			'description'   => __( 'Add widgets here to appear in your footer.', 'new-theme' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
			'before_title'  => '<h2 class="widget-title">',
			'after_title'   => '</h2>',
...
add_action("widgets_init", "supprimer_sidebar_1");

function supprimer_sidebar_1(){
